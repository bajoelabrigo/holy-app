...
function ActivityDetails() {
  ...
  const editPetition = useMutation({
    ...
    onSuccess: (_, { petitionId, petitionText }) => {
      queryClient.setQueryData(["petitions", activityId], (old) =>
        old.map((p) => (p._id === petitionId ? { ...p, petitionText } : p))
      );
  ...
  return (
    <>
      <h1 className="text-3xl font-bold mb-4 text-center">{activity.title}</h1>
        ...
        <ul className="mt-4 space-y-3">
          {petitions.map((petition) => {
            ...
            return (
              ...
              >
                <img
                  src={petition.userId?.profilePicture || "/profile.png"}
      ...
      <div className="mt-8 grid lg:grid-cols-2 gap-4 lg:w-6xl">
        <Activities />
      </div>
    </>
  );
